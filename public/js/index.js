...
socket.on('play', (ennemyPlayer) => {
    if(ennemyPlayer.socketId != player.socketId && !ennemyPlayer.turn) {
        ...
        if (ennemyPlayer.win) {
            SetTurnMessage('alert-info', 'allert-danger', `it's lost! <b>${ennemyPlayer.username}</b> win`);
            calculateWin(ennemyPlayer.playedCell, 'O');
            return;
        }

        if(calculateEquality()) {
            SetTurnMessage('alert-info', 'allert-warning', `It's an equality`);
            return;
        }

        SetTurnMessage('allert-info', 'alert-success', `It's your turn`);
        player.turn = true;
    } else {
        if (player.win) {
            $("#turn-message").addClass('allert-success').html("Congratulations! You win!");
            return;
        }
        ...
        SetTurnMessage('allert-success', 'alert-info', `It's <b>${ennemyPlayer.username}</b> turn`);
    }
});
